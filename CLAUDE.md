...
- **singleInstance** — `pg_try_advisory_lock(hashtext(name))` on a dedicated `pg.Client`; client held for the life of the loop; released on `stop()` / `lockClient.end()`
- **`applyProjectionSchema` not exported** — `ProjectionManager` owns schema end-to-end; split ownership causes inconsistency risk
